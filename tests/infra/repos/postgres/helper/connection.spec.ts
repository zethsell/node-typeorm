...
import { createConnection, getConnection, getConnectionManager, getRepository, getCustomRepository } from 'typeorm'
...
describe('PgConnection', () => {
  ...
  let getCustomRepositorySpy: jest.Mock
  ...
  beforeAll(() => {
    ...
    getCustomRepositorySpy = jest.fn().mockReturnValue('any_repo')
    createQueryRunnerSpy = jest.fn().mockReturnValue({
      startTransaction: startTransactionSpy,
      commitTransaction: commitTransactionSpy,
      rollbackTransaction: rollbackTransactionSpy,
      release: releaseSpy,
      manager: { getRepository: getRepositorySpy, getCustomRepository: getCustomRepositorySpy }
    })
    ...
    mocked(getCustomRepository).mockImplementation(getCustomRepositorySpy)
  })
  ...
  it('should get custom repository from queryRunner', async () => {
    await sut.connect()
    await sut.openTransaction()
    const repository = sut.getCustomRepository(User)

    expect(getCustomRepositorySpy).toHaveBeenCalledWith(User)
    expect(getCustomRepositorySpy).toHaveBeenCalledTimes(1)
    expect(repository).toBe('any_repo')
  ...
  it('should return ConnectionNotFoundError on getCustomRepository if connection is not found', async () => {
    expect(getCustomRepositorySpy).not.toHaveBeenCalled()
    expect(() => sut.getCustomRepository(User)).toThrow(new ConnectionNotFoundError())
  })
})
